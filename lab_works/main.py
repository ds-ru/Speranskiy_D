...
def notify_admin(request_number, user_id, username, message, timestamp):
    notification_text = (
...
def main(message):
    ...
    1. Бот *не* принимает файлы, их можно будет отправить после подачи заявки, когда мы с Вами свяжемся.
    
    2. При допущении *ошибки* Вы можете закрыть текущую заявку, нажав на соответствующую кнопку и отправить описание задачи заново."""
        ...
        bot.reply_to(message, "Вы превысили лимит запросов. Попробуйте снова позже.")
...
def view_requests(message):
    if message.from_user.id == ADMIN_ID:
        if requests:
            ...
            bot.reply_to(message, f"Все заявки:\n{requests_list}")
        else:
            bot.reply_to(message, "Заявок нет.")
    else:
        bot.reply_to(message, "У вас нет прав для просмотра заявок.")
...
def prices(message):
    if check_rate_limit(message.from_user.id):
        ...
        bot.send_message(message.chat.id, price_list, parse_mode='MarkdownV2')
    else:
        bot.reply_to(message, "Вы превысили лимит запросов. Попробуйте снова позже.")
...
@bot.message_handler(func=lambda message: message.text not in ["Полное описание", "Закрыть заявку", "Прайс-лист"])
    ...
    if check_rate_limit(message.from_user.id):
        if message.from_user.id != ADMIN_ID:
            ...
            if active_request is None:
                ...
                notify_admin(request_number, user_id, "@" + username, message.text, timestamp)
                ...
                bot.send_message(user_id, "Ваше сообщение добавлено к текущей заявке.")

            save_requests()  # Сохраняем заявки в файл
        else:
            bot.reply_to(message, "Работаю в поте лица, Сэр!")
    else:
        bot.reply_to(message, "Вы превысили лимит запросов. Попробуйте снова позже.")
...
def close_request(message):
    if check_rate_limit(message.from_user.id):
        ...
        for req in requests.values():
            if req['user_id'] == user_id and req['active']:
                req['active'] = False
                bot.reply_to(message, "Ваша заявка успешно закрыта.")
                save_requests()
                break
        else:
